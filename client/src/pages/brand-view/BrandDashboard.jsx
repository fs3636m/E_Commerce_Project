...
import { fetchBrandById } from "@/store/brand/getbrandbyid_slice";
  ...
  useEffect(() => {
    const fetchBrand = async () => {
      try {
        ...
        // If brand not found, try fetching via fetchBrandById
        if (err?.response?.status === 404) {
          try {
            const fallbackBrand = await dispatch(fetchBrandById("default")).unwrap();
            setBrand(fallbackBrand);
          } catch (fallbackErr) {
            setError("No brand found. You can create your brand now!");
          }
        } else {
  ...
  if (error)
    return (
      <div className="p-4 text-center">
        ...
        <Button onClick={() => navigate("/brand/create")}>Create Your Brand</Button>
      </div>
    );
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-4">
          <img
            ...
            alt={brand.name || "New Brand"}
            className="w-24 h-24 rounded-full object-cover border"
          />
          <div className="flex-1 w-full space-y-2 text-center md:text-left">
            <h1 className="text-2xl font-bold">{brand.name || "Your New Brand"}</h1>
            <p className="text-sm text-muted-foreground">
              {brand.bio || "Start by creating your brand bio and share your story!"}
            </p>
            ...
            <div className="flex justify-center md:justify-start mt-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => navigate(brand._id ? "/brand/edit-profile" : "/brand/create")}
              >
                <PencilIcon className="w-4 h-4 mr-2" />
                {brand._id ? "Edit Profile" : "Create Brand"}
              </Button>
            </div>
          </div>
        </div>
